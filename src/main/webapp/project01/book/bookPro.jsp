...
<jsp:useBean id="bookdto" class="web.bean.dto.BookDTO" />
...
<%
    ...
    int people = bookDAO.getPeoples(exhibit.getExhibit_name(),date,time)+quantity;
    int totalSeat = 20;
    int remain = totalSeat - people;
    
    if(remain < 0){
%>		<script>
			alert("선택하신 시간대에 관람 가능한 인원은 "+<%=remain+quantity%>+"명 입니다.");
			history.go(-1);
		</script>	
<%    }
%>
	...
	<form action="paymentForm.jsp?exhibit_number=<%=exhibit.getExhibit_number()%>" method="post">
		<input type="hidden" name="date" value="<%=date %>">
		<input type="hidden" name="time" value="<%=time %>">
		<input type="hidden" name="quantity" value="<%=quantity %>">
        <button type="submit">결제페이지로 이동</button>
